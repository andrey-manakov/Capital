...
// MARK: - Extentension creates simple way of adding subViews to view easily with one command using Visual Format

extension UIView {

    func add(subViews views: [String : UIView?], withConstraints constraints: Array<String>) {
        guard let views = views as? [String: UIView] else {print("error in addSubviewsWithConstraints");return}
        ...
        for i in 0..<constraints.count {
            self.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: constraints[i], options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: views))
        }
    }
    
    func add(subView view: UIView?, withConstraints constraints: Array<String>) {
...
extension Date {
    var str: String {return DateFormatter("yyyy MMM-dd").string(from: self)}
