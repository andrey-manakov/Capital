...
protocol FIRFinTransactionManagerProtocol_old: class {
    func createTransaction(from: String?, to: String?, amount: Int?, date: Date?, approvalMode: FinTransaction.ApprovalMode?, recurrenceFrequency: RecurrenceFrequency?, recurrenceEnd: Date?, completion: ((String?)->())?)
...
extension FIRFinTransactionManagerProtocol_old {
...
extension FIRFinTransactionManager_old: FireStoreCompletionProtocol, FireStoreGettersProtocol, MiscFunctionsProtocol {}

class FIRFinTransactionManager_old: FIRManager, FIRFinTransactionManagerProtocol_old {
    /// Singlton
    static var shared: FIRFinTransactionManagerProtocol_old = FIRFinTransactionManager_old()
    ...
    func createTransaction(from: String?, to: String?, amount: Int?, date: Date? = Date(), approvalMode: FinTransaction.ApprovalMode? = nil, recurrenceFrequency: RecurrenceFrequency? = nil, recurrenceEnd: Date? = nil, completion: ((String?)->())? = nil) {
    ...
    func sendFinTransaction(to fsTransaction: Transaction, from: AccountInfo?, to: AccountInfo?, amount: Int?, date: Date? = Date(), approvalMode: FinTransaction.ApprovalMode? = nil, recurrenceFrequency: RecurrenceFrequency? = nil, recurrenceEnd: Date? = nil, parent: String? = nil, approvedAmount: Int = 0) -> Int {
        ...
        fsTransaction.setData([
            FinTransaction.Fields.from.rawValue :
                [FinTransaction.Fields.From.id.rawValue: from.id, FinTransaction.Fields.From.name.rawValue: from.name] as Any,
