protocol FIRFinTransactionManagerProtocolOld: class {
    ...
    func sendFinTransaction(
        ...
        approvalMode: FinTransaction.ApprovalMode?, recurrenceFrequency: RecurrenceFrequency?,
        recurrenceEnd: Date?, parent: String?, approvedAmount: Int) -> Int
}
...
extension FIRFinTransactionManagerProtocolOld {
    ...
    func sendFinTransaction(
        ...
        return sendFinTransaction(
            to: fsTransaction, from: from, to: to,
            amount: amount, date: nil, approvalMode: nil,
            recurrenceFrequency: nil, recurrenceEnd: nil,
            parent: nil, approvedAmount: 0)
    }
}
...
class FIRFinTransactionManagerOld: FIRManager, FIRFinTransactionManagerProtocolOld {
        ...
        db.runTransaction({ (fsTransaction, errorPointer) -> Any? in
            ...
            let approvedAmount =
                self.sendFinTransaction(
                    to: fsTransaction,
                    from: (from, fromAccount.name ?? ""),
                    to: (to, toAccount.name ?? ""),
                    amount: amount,
                    date: date,
    ...
    ///   - completion: action to perform after function finishes execution,
    ///     for now it only works for successes
    ///   - parent: id of parent transaction -
    ///     transaction which is based for recurrent transactions,
    ///     this parent prpoerty is used to be able to get the group of recurrent transactions
    ///   - approvedAmount: amount returned - the sum of amounts of approved transactions
    ///     (in the past) used to update account values
    /// - Returns: approvedAmount(Int) - the sum of amounts of approved transactions
    ///     (in the past) used to update account values
    func sendFinTransaction(
        to fsTransaction: Transaction, from: AccountInfo?, to: AccountInfo?,
        amount: Int?, date: Date? = Date(), approvalMode: FinTransaction.ApprovalMode? = nil,
        recurrenceFrequency: RecurrenceFrequency? = nil, recurrenceEnd: Date? = nil,
        parent: String? = nil, approvedAmount: Int = 0) -> Int {

        guard let newFinTransactionRef =
            self.ref?.collection(DataObjectType.transaction.rawValue).document(),
        ...
        if let rf = recurrenceFrequency, rf != .never,
            ...
            return sendFinTransaction(to: fsTransaction, from: from, to: to,
                                      amount: amount, date: nextDate, approvalMode: approvalMode,
                                      recurrenceFrequency: recurrenceFrequency,
                                      recurrenceEnd: recurrenceEnd,
