...
protocol FIRAccountGroupManagerProtocol: class {
    ...
    func delete(id: String, completion: (()->())?)
}
...
class FIRAccountGroupManager: FIRManager, FIRAccountGroupManagerProtocol {
    ...
    func create(_ name: String?, withAccounts accountIds: [String]) {
        guard let ref = ref, let name = name, let newRef = self.ref?.collection(DataObjectType.group.rawValue).document() else {return}
        db.runTransaction({ (fsTransaction, errorPointer) -> Any? in
            // get accounts from the group
            let accounts: [String: Account] = Dictionary(uniqueKeysWithValues: accountIds.map{(id: $0, (self.getAccount(withId: $0, for: fsTransaction, with: errorPointer))!)})//FIXME: !
            //FIXME: revise calc of amount
            let amount = accounts.values.filter{$0.type == AccountType.asset}.map{$0.amount ?? 0}.reduce(0, +) - accounts.values.filter{$0.type == AccountType.liability}.map{$0.amount ?? 0}.reduce(0, +)
            ...
            accountIds.forEach {id in
                ...
                if let oldAccountGroup = accounts[id]?.groups {
                    newAccountGroup = newAccountGroup.merging(oldAccountGroup, uniquingKeysWith: { x, _ -> String in x})
                }
                fsTransaction.updateData([Account.Fields.groups.rawValue : newAccountGroup as Any], forDocument: ref.collection(DataObjectType.account.rawValue).document(id))
            }
            // create account group
            fsTransaction.setData([
                Account.Group.Fields.name.rawValue : name,
                Account.Group.Fields.amount.rawValue: amount,
                Account.Group.Fields.accounts.rawValue: accounts.mapValues{acc in acc.name}], forDocument: newRef)
            return true
    ...
    func delete(id: String, completion: (()->())? = nil) {
        guard let ref = ref else {return}
        db.runTransaction({[unowned self] (fsTransaction, errorPointer) -> Any? in
            // Get account group data
            guard let group = self.get(.group, withId: id, for: fsTransaction, with: errorPointer) as? Account.Group else {return nil}
            // Get accounts data
            if let uniqueKeysWithValues = group.accounts?.keys.map({(id: $0, self.get(.account, withId: $0, for: fsTransaction) as? Account)}) {
                ...
                accounts.forEach({
                    if let groups = $0.value?.groups {
                        ...
                        fsTransaction.updateData([Account.Fields.groups.rawValue : newGroups], forDocument: ref.collection(DataObjectType.account.rawValue).document($0.key))
                    }
                })
            }
